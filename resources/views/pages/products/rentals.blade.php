  ...
  <div class="flex min-w-0 flex-1 flex-col overflow-hidden">
    ...
    <main class="flex-1 overflow-y-auto bg-[#F9FAFB] p-4 sm:p-6">
      <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h1 class="text-[21px] font-semibold tracking-[-0.02em] text-[#101828]">Rental Products</h1>
          <p class="mt-1 text-[12.5px] text-[#4A5565]">Yahan sirf rental products show honge. Ye page database se live sync rahega.</p>
        </div>
        <a href="{{ route('products.create') }}" class="inline-flex items-center justify-center gap-2 rounded-xl border border-red-500 bg-red-500 px-4 py-2.5 text-[13.5px] font-medium text-white transition hover:bg-red-600">
        ...
        @foreach ($products as $product)
          @php
            ...
            $viewPayload = [
              ...
              'price' => $product['price'] ? 'Rs ' . number_format($product['price'], 0) : 'N/A',
              'rent_price' => $product['rent_price'] ? 'Rs ' . number_format($product['rent_price'], 0) . '/' . ($product['billing_period'] ?? 'month') : 'N/A',
              'image' => $product['image_path'] ? asset($product['image_path']) : null,
              'features' => $product['features'] ?? [],
              'specifications' => $product['specifications'] ?? [],
            ];
          @endphp
          <div class="product-card overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-soft" data-name="{{ strtolower($product['name']) }}" data-category="{{ $product['category'] }}" data-status="{{ $product['status'] }}">
            ...
            <div class="p-4">
              ...
              <div class="mt-4 flex items-center gap-2">
                ...
                <div class="ml-auto text-right">
                  <div class="text-[12px] font-bold text-[#101828]">Rs {{ number_format($product['rent_price'] ?? 0, 0) }}/{{ $product['billing_period'] ?? 'month' }}</div>
                  @if ($product['price'])
                    <div class="text-[10px] text-slate-400">Buy: Rs {{ number_format($product['price'], 0) }}</div>
                  @endif
                </div>
              </div>
            </div>
...
<div id="productFormModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 modal-overlay">
  <div class="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
    <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
      <div>
        <div class="text-[15px] font-semibold text-[#101828]">Edit Rental Product</div>
        <div class="text-[11.5px] text-[#4A5565]">Update rental product details, pricing, image, and content.</div>
      </div>
    ...
    <form id="productForm" method="POST" action="{{ $formProduct ? route('products.update', $formProduct['id']) : '#' }}" enctype="multipart/form-data" class="p-6">
      @csrf
      @method('PUT')
      ...
      <div class="mt-6 flex items-center justify-end gap-3 border-t border-slate-100 pt-4">
        ...
        <button type="submit" class="rounded-xl bg-brand px-5 py-2.5 text-[13px] font-semibold text-white transition hover:bg-red-500">Save Changes</button>
      </div>
    </form>
  </div>
</div>
