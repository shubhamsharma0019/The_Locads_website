...
<head>
  ...
  @php
    ...
    $pageTitle = $isTicketActive ? 'Ticket Enquiry' : 'Enquiries';
  @endphp
  <style>
  ...
  <div class="flex min-w-0 flex-1 flex-col overflow-hidden">
    ...
    <main class="flex-1 overflow-y-auto bg-[#F9FAFB] p-4 sm:p-6">
      <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h1 class="text-[21px] font-medium tracking-[-0.02em] text-[#101828]">{{ $pageTitle }}</h1>
      ...
      <div id="enquiriesGrid" class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
        @foreach ($enquiries as $enquiry)
          @php
            ...
            $feedbackPreview = \Illuminate\Support\Str::limit($enquiry['feedback_remark'] ?? '', 90);
          ...
          <div class="enquiry-card overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-soft" data-name="{{ strtolower($fullName) }}" data-email="{{ strtolower($enquiry['email']) }}" data-company="{{ strtolower($enquiry['company_name'] ?? '') }}" data-source="{{ strtolower($enquiry['source_page']) }}" data-type="{{ $enquiry['enquiry_type'] }}">
            ...
            <div class="p-4">
              ...
              <div class="mt-3 text-[11px] font-medium uppercase tracking-wide text-slate-400">Response / Solution</div>
              <div class="mt-1 min-h-[38px] text-[12px] leading-5 text-[#4A5565]">{{ $feedbackPreview ?: 'No response added yet.' }}</div>
  ...
  <div class="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
    ...
    <form id="enquiryEditForm" method="POST" action="#" class="grid gap-4 p-6 md:grid-cols-2">
      ...
      <div class="md:col-span-2">
        <label for="editMessage" class="text-[11px] font-medium uppercase tracking-wide text-slate-400">Original Query</label>
        <textarea id="editMessage" rows="5" class="mt-1 w-full rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm leading-6 text-[#4A5565] outline-none" readonly></textarea>
      </div>
      <div class="md:col-span-2">
        <label for="editFeedbackRemark" class="text-[11px] font-medium uppercase tracking-wide text-slate-400">Response / Solution</label>
        <textarea id="editFeedbackRemark" name="feedback_remark" rows="4" class="mt-1 w-full rounded-2xl border border-slate-200 px-3 py-2.5 text-sm leading-6 outline-none focus:border-brand" maxlength="2000" placeholder="Add the response or solution given for this query"></textarea>
      </div>
...
<div id="enquiryViewModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 modal-overlay">
  <div class="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
    ...
    <div class="grid gap-4 p-6 md:grid-cols-2">
      ...
      <div class="md:col-span-2">
        <div class="text-[11px] font-medium uppercase tracking-wide text-slate-400">Response / Solution</div>
        <div id="modalFeedbackRemark" class="mt-1 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-[13px] leading-6 text-[#4A5565]">-</div>
      </div>
    </div>
  </div>
</div>
...
<script>
  ...
  function openEnquiryModal(enquiry) {
    ...
    document.getElementById('modalFeedbackRemark').textContent = enquiry.feedback_remark || 'No response added yet.';
  ...
  function openEditEnquiryModal(enquiry) {
    ...
    document.getElementById('editFeedbackRemark').value = enquiry.feedback_remark || '';
